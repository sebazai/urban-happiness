...
fn main() {
    ...
    coordinates.iter().for_each(|&((x1, y1), (x2,y2))| {
        if x1 == x2 {
            ...
            // as we have 45 degrees, the distance is same when going diagonal
            let distance = (x1 - x2).abs();
            // Start at left most point of x-axis
            let starting_point = if x1 < x2 {
                // (5,5 -> 8,2) & (0,0) -> (8,8)
                (x1, y1)
            } else {
                // (9,7) -> (7, 9) & (6,4) -> (2,0)
            ...
            for i in 0..=distance {
                // if we descend or ascend
                let coordinate = if (x1 < x2 && y1 < y2) || (x1 > x2 && y1 > y2) {
                    (starting_point.0 + i, starting_point.1 + i)
                } else {
                    (starting_point.0 + i, starting_point.1 - i)
                };
                vents.insert(coordinate, vents.get(&coordinate).unwrap_or(&0).clone() + 1);
